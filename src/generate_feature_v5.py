...
def extract_features_xg(train_file, test_file, feat_train_file, feat_test_file):
    ...
    feat = df.groupby('category_lvl_1')['title'].apply(lambda x: ' '.join(x))
    feat = feat.reset_index()
    
    feat['cat1_word_cnt'] = feat['title'].map(lambda x: np.log(len(x.split(' '))))
    feat['cat1_word_nunique'] = feat['title'].map(lambda x: np.log(len(set(x.split(' ')))))
    
    feat1 = df.groupby('category_lvl_2')['title'].apply(lambda x: ' '.join(x))
    feat1 = feat1.reset_index()
    
    feat1['cat2_word_cnt'] = feat1['title'].map(lambda x: np.log(len(x.split(' '))))
    feat1['cat2_word_nunique'] = feat1['title'].map(lambda x: np.log(len(set(x.split(' ')))))
    
    feat3 = df.groupby('category_lvl_3')['title'].apply(lambda x: ' '.join(x))
    feat3 = feat3.reset_index()
    
    feat3['cat3_word_cnt'] = feat3['title'].map(lambda x: np.log(len(x.split(' '))))
    feat3['cat3_word_nunique'] = feat3['title'].map(lambda x: np.log(len(set(x.split(' ')))))
    ...
    df['title_word_cnt'] = df['title'].map(lambda x: len(x.split(' ')))
    df['title_word_nunique'] = df['title'].map(lambda x: len(set(x.split(' '))))
    
    feat = df.groupby(['category_lvl_1','category_lvl_2'])['title_word_cnt'].agg([np.min, np.max, np.mean, np.median, np.std])
    feat.columns = ['cat1_word_cnt_min','cat1_word_cnt_max','cat1_word_cnt_mean','cat1_word_cnt_median','cat1_word_cnt_std']
    feat = feat.reset_index()
    df = df.merge(feat, how='left', on=['category_lvl_1','category_lvl_2'])
    feat_names.extend(['cat1_word_cnt_min','cat1_word_cnt_max','cat1_word_cnt_mean','cat1_word_cnt_median','cat1_word_cnt_std'])
    
    feat = df.groupby(['category_lvl_2','category_lvl_3'])['title_word_cnt'].agg([np.min, np.max, np.mean, np.median, np.std])
    feat.columns = ['cat2_word_cnt_min','cat2_word_cnt_max','cat2_word_cnt_mean','cat2_word_cnt_median','cat2_word_cnt_std']
    feat = feat.reset_index()
    df = df.merge(feat, how='left', on=['category_lvl_2','category_lvl_3'])
    feat_names.extend(['cat2_word_cnt_min','cat2_word_cnt_max','cat2_word_cnt_mean','cat2_word_cnt_median','cat2_word_cnt_std'])
    
    feat = df.groupby(['category_lvl_3','category_lvl_1'])['title_word_cnt'].agg([np.min, np.max, np.mean, np.median, np.std])
    feat.columns = ['cat3_word_cnt_min','cat3_word_cnt_max','cat3_word_cnt_mean','cat3_word_cnt_median','cat3_word_cnt_std']
    feat = feat.reset_index()
    df = df.merge(feat, how='left', on=['category_lvl_3','category_lvl_1'])
    feat_names.extend(['cat3_word_cnt_min','cat3_word_cnt_max','cat3_word_cnt_mean','cat3_word_cnt_median','cat3_word_cnt_std'])
    
    feat = df.groupby(['category_lvl_1','category_lvl_2','category_lvl_3'])['title_word_cnt'].agg([np.min, np.max, np.mean, np.median, np.std])
    feat.columns = ['cat4_word_cnt_min','cat4_word_cnt_max','cat4_word_cnt_mean','cat4_word_cnt_median','cat4_word_cnt_std']
    feat = feat.reset_index()
    df = df.merge(feat, how='left', on=['category_lvl_1','category_lvl_2','category_lvl_3'])
    feat_names.extend(['cat4_word_cnt_min','cat4_word_cnt_max','cat4_word_cnt_mean','cat4_word_cnt_median','cat4_word_cnt_std'])
...
if __name__ == "__main__":
    ...
    feat_train_file = '../features/cat_cnt_feat_log.trn'
    feat_test_file = '../features/cat_cnt_feat_log.tst'
