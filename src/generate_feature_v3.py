...
def extract_features_xg(train_file, test_file, feat_train_file, feat_test_file):
    ...
    feat['cat1_word_cnt'] = feat['title'].map(lambda x: len(x.split(' ')))
    feat['cat1_word_nunique'] = feat['title'].map(lambda x: len(set(x.split(' '))))
    
    feat1 = df.groupby('category_lvl_2')['title'].apply(lambda x: ' '.join(x))
    feat1 = feat1.reset_index()
    
    feat1['cat2_word_cnt'] = feat1['title'].map(lambda x: len(x.split(' ')))
    feat1['cat2_word_nunique'] = feat1['title'].map(lambda x: len(set(x.split(' '))))
    
    feat3 = df.groupby('category_lvl_3')['title'].apply(lambda x: ' '.join(x))
    feat3 = feat3.reset_index()
    
    feat3['cat3_word_cnt'] = feat3['title'].map(lambda x: len(x.split(' ')))
    feat3['cat3_word_nunique'] = feat3['title'].map(lambda x: len(set(x.split(' '))))
...
if __name__ == "__main__":
    ...
    feat_train_file = '../features/cat_cnt_feat.trn'
    feat_test_file = '../features/cat_cnt_feat.tst'
