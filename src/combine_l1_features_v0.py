...
if __name__ == '__main__':
    ...
    list_file = [x for x in os.listdir('../pred') if x.endswith('full.csv')]
    ...
    for f in list_file:
        ...
        file_names.append(gen_l2_feature(model_name=model_name, feat_name=feat_name, label_name=label_name, pred_folder='pred'))
    ...
    df_tr.to_csv('../features/conciseness.L1_FoldSetX.TRAINSET.csv' , index=False)        
    df_va.to_csv('../features/conciseness.L1_FoldSetX.TESTSET.full.csv' , index=False)        
    for k in range(10):
        df_te = df_te_set[k]
        df_va.to_csv('../features/conciseness.L1_FoldSetX.TESTSET.fold%d.csv' % k, index=False) 
